...
static int ahmet(const char *s, char c);
...
static int ahmet(const char *s, char c) //kelimelerin uzunlugu aliniyor.
{
	int counter;
...
/*int	main(void)
{
	const char array[] = ",,,,  , Ahmet,Mehmet,,,,, Mustafa ";
	int i = 0;
	char **a;
	a = ft_split(array, ',');
	while(a[i] != NULL)
